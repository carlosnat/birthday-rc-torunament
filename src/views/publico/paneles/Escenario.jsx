...
// layout.
//
// Durante la carrera queda libre a propósito (pedido del diseño). El QR y el podio sí viven
// acá porque no tienen otro lugar: sin ellos no se puede registrar ni cerrar el torneo.
...
export default function Escenario({ torneo, sesion, orden, equipos, standings, sonido }) {
  if (torneo.estado === TORNEO.REGISTRO) {
    return (
      <section className="esc esc--qr">
  ...
  if (sesion?.estado === SESION.BANDERA) {
    return <section className="esc"><BanderaCuadros /></section>
  }
  ...
  if (sesion && (sesion.estado === SESION.LARGADA || (sesion.estado === SESION.EN_CURSO && !arrancado))) {
    return <section className="esc"><SemaforoF1 estado={sesion.estado} sonido={sonido} /></section>
  }

  return <section className="esc esc--libre" />
}

function PanelQR({ titulo, rol }) {
  return (
    <div className="esc-qr">
      ...
      <div className="esc-qr-cta">ESCANEÁ Y SUMATE</div>
    </div>
  )
}
